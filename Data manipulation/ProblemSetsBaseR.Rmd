...
Check your data with calls to built-in R functions. `dim` returns number of row and number of columns. We have many columns (n=`r ncol(CBdata)`).
...
**Step 3**: A call to `base::prop.table` should do the trick. `margin = 1` indicates by rows (i.e., by year). *Note*: If not using a call to `t` like above, then set `margin = 2` for colums.
